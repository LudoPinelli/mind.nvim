...
M.save_local_state = function()
  if M.local_tree ~= nil then
    ...
    if M.local_cwd and cwd ~= M.local_cwd then
      notify(
        "refusing to save local state: differs from when it was loaded",
        vim.log.levels.ERROR
      )
      notify(
        string.format("hint: loaded as %s, try saving as %s", M.local_cwd, cwd),
        vim.log.levels.INFO
      )
      notify(
        string.format("hint: cd to %s in order to save the tree", M.local_cwd),
        vim.log.levels.INFO
      )
      return
    end
